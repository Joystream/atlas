  ...
  const reactToComment = useCallback(
    async (commentId: string, videoId: string, reactionId: CommentReaction, commentAuthorHandle: string, fee?: BN) => {
      ...
      if (commentId.includes(UNCONFIRMED)) {
        displaySnackbar({
          title: "Couldn't react to comment",
          description: 'Looks like this comment was not yet confirmed by server. Please retry shortly.',
        })
        return
      }
    ...
    [joystream, memberId, handleTransaction, displaySnackbar, proxyCallback, refetchComment, refetchReactions]
  )
