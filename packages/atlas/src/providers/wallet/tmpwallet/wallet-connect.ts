...
export class WalletConnectWallet implements BaseWallet {
  ...
  async connect() {
    ...
    const signerChainId = Array.isArray(this.config.chainIds) ? this.config.chainIds[0] : JOYSTREAM_DEFAULT_CHAIN_ID

    if (lastSession) {
      return new Promise<void>((resolve) => {
        this.session = lastSession

        this.signer = new WalletConnectSigner(this.client!, lastSession, signerChainId)
        resolve()
      })
    }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      approval()
        .then((session) => {
          ...
          this.signer = new WalletConnectSigner(this.client!, session, signerChainId)
